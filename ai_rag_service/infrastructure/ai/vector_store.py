...
class FAISSVectorStoreAdapter(VectorStorePort):
    ...
    def __init__(self, embedding_model: str = "sentence-transformers/all-MiniLM-L6-v2"):
        ...
        # Read persistent path from env, fallback to local storage for dev
        ...
        try:
            ...
            logger.warning(f"⚠️ FAISS: No se pudo crear directorio de vector store: {e}")

        # Intentar cargar un vectorstore persistido en disco (si existe)
        try:
            self._load_vectorstore()
            if self.vector_store is not None:
                # Try to report number of vectors if available
                try:
                    n_vectors = getattr(self.vector_store.index, 'ntotal', None)
                    if n_vectors is not None:
                        logger.info(f"✅ FAISS: Índice cargado exitosamente con {n_vectors} vectores.")
                    else:
                        logger.info("✅ FAISS: Índice cargado exitosamente.")
                except Exception:
                    logger.info("✅ FAISS: Índice cargado exitosamente.")
            else:
                logger.warning("⚠️ FAISS: Índice local no encontrado. Iniciando RAG con memoria vacía.")
        except Exception as e:
            logger.warning(f"⚠️ FAISS: Error cargando índice local: {e}. Iniciando RAG con memoria vacía.")
    ...
    def unload(self):
        """Unload the vector store and free memory references."""
        try:
            if self.vector_store is not None:
            ...
            logger.info("✅ FAISS: Índice descargado de memoria")
        except Exception as e:
            logger.warning(f"⚠️ FAISS: Error al descargar índice: {e}")
