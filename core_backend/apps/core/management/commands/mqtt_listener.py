...
class Command(BaseCommand):
    ...
    def handle(self, *args, **options):
        ...
        broker_host = parsed_uri.hostname or 'mqtt_broker'
        broker_port = parsed_uri.port or (8883 if use_tls else 1883)

        self.stdout.write(self.style.NOTICE(f"Inicializando MQTT Listener en {broker_host}:{broker_port} (TLS={use_tls})..."))
        ...
        # Configurar TLS si está habilitado (Sprint 1)
        if use_tls:
            certs_path = '/mosquitto/certs'
            try:
                client.tls_set(
                    ca_certs=f"{certs_path}/ca.crt",
                    certfile=None,
                    keyfile=None,
                    tls_version=ssl.PROTOCOL_TLS_CLIENT
                )
                client.tls_insecure_set(False)  # Verify server cert
                self.stdout.write(self.style.SUCCESS(f"TLS configurado con CA: {certs_path}/ca.crt"))
            except Exception as e:
                self.stdout.write(self.style.ERROR(f"Error al configurar TLS: {str(e)}"))
                return
